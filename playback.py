...
def play_recording(npz_path, speed=1.0):
    ...
    try:
        ...
        if 'buttons' in data:
            ...
            buttons = ['UP', 'DOWN', 'LEFT', 'RIGHT', 'A', 'B', 'C', 'X', 'Y', 'Z', 'MODE', 'START', 'SELECT']
        ...
        pygame.init()
        window = pygame.display.set_mode((frames[0].shape[1] * 2, frames[0].shape[0] * 2))
        ...
        while running and frame_idx < len(frames):
            ...
            if not paused:
                ...
                action_text = " | ".join(f"{btn}:{int(act)}" for btn, act in zip(buttons, action))
                text_surface = font.render(f"Frame: {frame_idx}/{len(frames)} | {action_text}", True, (255, 255, 255))
                ...
                # Show paused screen
                window.fill((0, 0, 0))
                pause_text = font.render("PAUSED (SPACE=resume, LEFT/RIGHT=seek, ESC=quit)", True, (255, 255, 255))
